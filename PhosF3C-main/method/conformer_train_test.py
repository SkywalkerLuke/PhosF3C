...
# Training function
def train(lora_esm,tok,model,data_loader,opt,criterion,config):
    device=config.device
    print('\n-------------train------------------\n')
    # Log the number of positive and negative training samples
    print(f'positive number : {len(data_loader["p_t"].dataset)}')
    print(f'negative number : {len(data_loader["n_t"].dataset)}')
    ...
    for epoch in tqdm(range(config.num_epoch),desc='epoch',total=config.num_epoch):
        ...
        # Set LoRA-ESM to embedding mode and model to training mode
        lora_esm.eval()
        model.train()
        
        # Loop through positive and negative training batches
        for (p_train_seq_list, p_train_list), (n_train_seq_list, n_train_list) in tqdm(zip(data_loader["p_t"], data_loader["n_t"]), desc='step', total=len(data_loader["p_t"])):
            ...
            if it%config.val_interval==0:
                val_info=validate(lora_esm,tok,model,data_loader,config,it)
...
# Validation function
def validate(lora_esm,tok,model,data_loader,config,it):
    device=config.device
    print('\n-------------validate------------------\n')
    print(f'step {it}:')
    ...
    with torch.no_grad():
        ...
        for p_val_seq_list, p_val_list in data_loader['p_v']:
            ...
            # Forward pass through model for positive samples
            p_esm_dict = lora_esm(p_val_num)
            p_hidden_representation = p_esm_dict['hidden']
            p_output_dict = model(p_hidden_representation)
            p_outputs = config.alpha * p_output_dict['conv_cls'] + (1 - config.alpha) * p_output_dict['tran_cls']
            _, predict = torch.max(p_outputs, 1)
            predict_labels = p_val_labels[:, 1:].view(-1)  # Extract true class labels
            tp, fp, fn, tn = calculate_metrics(predict, predict_labels)
            vTP += tp
            vFP += fp
            vTN += tn
            ...
            prob_list += p_outputs.tolist()
            label_list += p_val_labels.tolist()
        ...
        for n_val_seq_list, n_val_list in data_loader['n_v']:
            ...
            # Forward pass through model for negative samples
            n_esm_dict = lora_esm(n_val_num)
            n_hidden_representation = n_esm_dict['hidden']
            n_output_dict = model(n_hidden_representation)
            n_outputs = config.alpha * n_output_dict['conv_cls'] + (1 - config.alpha) * n_output_dict['tran_cls']
            ...
            prob_list += n_outputs.tolist()
            label_list += n_val_labels.tolist()
        ...
        return {'acc': acc, 'auc': auc, 'vTP': vTP, 'vFP': vFP, 'vTN': vTN, 'vFN': vFN, 'prob': prob_list, 'label': label_list}


# Testing function
def test(lora_esm, tok, model, data_loader, config):
    device = config.device
    print('\n-------------test------------------\n')
    ...
    with torch.no_grad():  # No gradient computation during testing
        ...
        for p_val_seq_list, p_val_list in tqdm(data_loader['p'], desc='positive test', total=len(data_loader['p'])):
            ...
            # Get the embeddings and output for the positive samples
            p_esm_dict = lora_esm(p_val_num)
            p_hidden_representation = p_esm_dict['hidden']
            p_output_dict = model(p_hidden_representation)

            # Compute outputs using weighted average of CNN and transformer outputs
            p_outputs = config.alpha * p_output_dict['conv_cls'] + (1 - config.alpha) * p_output_dict['tran_cls']
        ...
        for n_val_seq_list, n_val_list in tqdm(data_loader['n'], desc='negative test', total=len(data_loader['n'])):
            ...
            # Get the embeddings and output for the negative samples
            n_esm_dict = lora_esm(n_val_num)
            n_hidden_representation = n_esm_dict['hidden']
            n_output_dict = model(n_hidden_representation)
...
# Training wrapper
def train_conformer(train_ds, config):
    device = config.device
    ...
    log = train(lora_esm, esm_tokenizer, model, data_loader, opt, criterion, config)
    return log


# Testing wrapper
def test_conformer(test_ds, config):
    device = config.device
    ...
    model.load_state_dict(torch.load(config.conformer_checkpoint, map_location={'cpu': device}), strict=False)
    ...
    test_metrics = test(lora_esm, esm_tokenizer, model, data_loader, config)
    return test_metrics
