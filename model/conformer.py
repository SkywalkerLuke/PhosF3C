...
class ConvTransBlock(nn.Module):
    ...
    def __init__(self, inplanes, outplanes, res_conv, stride, dw_stride, embed_dim, num_heads=12, mlp_ratio=4,
                 qkv_bias=False, qk_scale=None, drop_rate=0.2, attn_drop_rate=0.2, drop_path_rate=0.2,
                 last_fusion=False, num_med_block=0, groups=1):

        super(ConvTransBlock, self).__init__()
        expansion = 4
        self.cnn_block = ConvBlock(inplanes=inplanes, outplanes=outplanes, res_conv=res_conv, stride=stride, groups=groups,drop_block=nn.Dropout(0.75),drop_path=nn.Dropout(0.75))
...
class Conformer(nn.Module):
    ...
    def __init__(self, patch_size=32, in_chans=1, num_classes=2, base_channel=64, channel_ratio=8, num_med_block=0,
                 embed_dim=1280, depth=12, num_heads=16, mlp_ratio=4., qkv_bias=False, qk_scale=None,
        ...
        # self.mix_cls_head = nn.Linear(embed_dim+int(256 * channel_ratio), num_classes) if num_classes > 0 else nn.Identity()
    ...
    def forward(self, x):
        ...
        # 1 stage
        x = self.conv_1(x_base, return_x_2=False)       #cnn input

        x_t = torch.cat([cls_tokens, x_t], dim=1)
        x_t = self.trans_1(x_t)     #transformer input
        ...
        cat_repr = torch.cat([conv_repr,trans_repr],dim=-1)
        # cat_cls = self.mix_cls_head(cat_repr)
